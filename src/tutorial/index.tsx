...
const task3Checkpoints = [
  {
    ...
    steps: <>
      ...
      <Typography variant="subtitle2" gutterBottom>
        2. Click the
        <Select value="paragraph" size='small' sx={{
          mx: 1,
      ...
      <Typography variant="subtitle2">
        3. Select Heading 3 from the menu
      </Typography>
    </>,
    check: (editorState?: EditorState) => {
      let result = false;
      ...
      editorState.read(() => {
        editorState._nodeMap.forEach((node) => {
          if ($isTextNode(node) && node.getTextContent() === "This is a level 3 heading") {
            if (node.getParent()?.__tag == "h3") result = true;
          }
        });
      })
      return result;
  ...
  },
  {
    ...
    check: (editorState?: EditorState) => {
      ...
      editorState.read(() => {
        editorState._nodeMap.forEach((node) => {
          if ($isTextNode(node) && node.getTextContent() === "Nature is written in mathematical language.") {
            if (node.getParent()?.__type == "quote") result = true;
          }
        });
      })
      return result;
